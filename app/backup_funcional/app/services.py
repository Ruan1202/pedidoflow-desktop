    ...
    for item in itens:
        ...
        if item.get("tipo") == "meio_a_meio":
            ...
            cursor.execute("""
                ...
                (pedido_id, produto_id, quantidade, preco_unitario)
                VALUES (?, ?, ?, ?)
            """, (pedido_id, item["produto_id_2"], item["quantidade"], 0))
            ...
            cursor.execute("""
                ...
                (pedido_id, produto_id, quantidade, preco_unitario)
                VALUES (?, ?, ?, ?)
            """, (pedido_id, item["produto_id"], item["quantidade"], produto[4]))
...
def gerar_texto_pedido(pedido_id, cliente_id, itens, observacao="", forma_pagamento="Não informado"):
    ...
    for item in itens:
        ...
        if item.get("tipo") == "meio_a_meio":
            ...
            linhas.append(
                f"Pizza Meio a Meio x{item['quantidade']} = R$ {total_item:.2f}"
            )
...
def salvar_pedido_txt(texto, pedido_id):
    ...
    caminho = os.path.join(pasta, f"pedido_{pedido_id}.txt")
...
def ler_txt_pedido(pedido_id):
    caminho = os.path.join("pedidos_txt", f"pedido_{pedido_id}.txt")
